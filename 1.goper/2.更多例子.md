...
---
## 例子1：为组件添加配置参数
我们沿用[开始使用]()中的例子，为 Example 添加一个变量 ac，用来控制具体的逻辑：
```
type Example struct {
	http.HttpComponent
	ac int 
...
---
## 例子4：新增多个服务
沿用上面的例子，我们新建一个 ExampleHandler 结构体类型，添加 Ac 变量 ，为该类型添加方法 Handler ，把原来 Example 的 Handler 删除：
```
...
为 Example 新增 MergeConfig 方法合并配置项：
```
